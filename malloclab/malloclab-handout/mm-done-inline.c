...
static inline uint32_t PACK(size_t size, int alloc) {
    return (size | alloc);
}
...
static inline uint32_t GET4BYTES(void *p) {
    return *((uint32_t *)p);
}

//#define PUT4BYTES(p, val)(*((unsigned*) (p)) = (val))
static inline void PUT4BYTES(void *p, uint32_t val) {
    *((uint32_t *)p) = val;
}
...
static inline char *SUCCESSOR(void *bp) {
    return (char *)((char *)bp);
}

//#define PREDECESSOR(bp)   ((char*) ((char*)(bp) + DSIZE))
static inline char *PREDECESSOR(void *bp) {
...
header and footer mismatch\n", ptr);
	...
	if ((char *) GET(SUCCESSOR(ptr)) != NULL &&
	    (char *) GET(PREDECESSOR((void *)GET(SUCCESSOR(ptr)))) != ptr)
	    printf("Free block pointer %p's \
next pointer is inconsistent\n", ptr);

	if ((char *) GET(PREDECESSOR(ptr)) != NULL &&
	    (char *) GET(SUCCESSOR((void *)GET(PREDECESSOR(ptr)))) != ptr)
	    printf("Free block pointer %p's \
